...
1. **Engine purity:** nothing in `src/engine/` may touch the wall clock, `Math.random`, timers, env, filesystem, or network. `test/purity.test.ts` enforces by grep; don't weaken it.
...
- Balance numbers live ONLY in `TUNING` (src/engine/agents.ts). Tuning passes touch that object, nothing else.
