...
- **Expeditions (RPG layer)**: outfit an adventurer with gear and food bought on the exchange, delve a six-region node graph from the Lumbridge Plains to The Dragon's Maw, fight turn-based battles, and bring monster loot home to sell — drops mint through the same audited conservation ledger as everything else. Death keeps your 3 most valuable carried items; the rest feeds the depths
- **Verified leaderboards**: submit your best fortune at tick 2,000 — the server replays your entire command log through the same deterministic engine and posts the worth *it* computed. Determinism is the anti-cheat; a forged score cannot reproduce
