...
- **Trading**: ~~suggested-flip margin line~~ DONE 10j; ~~fair-value band~~ DONE 10m; ~~market-row sparkline~~ already built; ~~"best flips now" ranked strip~~ DONE 10v; ~~realized profit-by-item~~ DONE 11b (`realizedPnL`, FIFO + tax, ProfitPanel). Next idea: factor buy-limit/depth so TopFlips ranks by *realizable* profit; a one-click "place this flip"; widen the fill window or persist a lifetime per-item P&L (fills cap at 50 today → "recent" only).
