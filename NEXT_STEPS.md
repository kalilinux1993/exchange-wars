...
- **Social/retention**: ~~daily shared seed~~ DONE 10q; ~~"today" badge~~ DONE 10r; ~~daily-streak counter~~ DONE 10s (🔥 ember, `bumpStreak` reducer, localStorage). Next: a **streak-at-risk nudge** — when the streak is alive but today's daily is *unplayed* (and the player is on some other seed), show "🔥 N — play today's daily to keep it" as a call-to-action button (the FOMO half of the loop, currently the ember only shows once you're already on the daily); "today's board" framing on the leaderboard view itself; a "🏁 daily best" personal record per day.
