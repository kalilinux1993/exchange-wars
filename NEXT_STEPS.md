...
- **Feedback/idle**: ~~earning-rate cue~~ DONE 10x; ~~compact big-number formatting~~ DONE 10y (`fmtCompact`, masthead aggregates). ~~tiny worth sparkline beside the rate~~ DONE 11h (masthead net-worth trajectory). Next: extend `fmtCompact` to the away-bar gp gain + milestone targets if they read long; persist lifetime per-item P&L + open-position cost basis (replaces the 50-fill "recent" window — a meatier brick: needs a persistent FIFO lot structure on Game + normalizeGame default).
