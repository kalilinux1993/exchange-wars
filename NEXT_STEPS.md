...
- **HUD depth**: ~~"train to unlock" gear hints in the character sheet~~ DONE 10w (lockedUpgrades — held-but-gated upgrades show 🔒 Atk/Def N). Next: per-slot item ICONS in the equiplist (itemIcon pipeline exists); richer paperdoll (per-item silhouettes); inventory/equipment grid; OSRS-style icon sub-tabs if a room crowds.
