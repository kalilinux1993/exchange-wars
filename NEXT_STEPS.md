...
- **RPG content**: more event faces (each bends a still-untouched system — FINDINGS #64); ~~Field Forge event~~ + ~~Skarn (first shallow elite, Wilderness Ruins)~~ BUILT 11e/11g (**batched in PR #1, awaiting merge + verify-score redeploy**); a 9th region (needs a NEW resource to extort); ~~attack brew~~ DONE 10n; encounter depletion (only if an audit shows the TAS tail needs bounding). Idea: named elites for the other shallow regions (0–3) now that the machinery is proven 2-data-edits cheap.
