...
- **HUD depth**: ~~"train to unlock" gear hints~~ DONE 10w; ~~per-slot item ICONS in the equiplist~~ DONE 11k; ~~region danger readout (hardest foe pre-embark)~~ DONE 11l (`regionDanger`). Next: richer paperdoll (per-item silhouettes); inventory/equipment grid; show YOUR atk/def beside the region danger for a direct compare.
